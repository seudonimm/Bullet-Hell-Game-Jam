...
public class ECircleEnemyController : MonoBehaviour
{
    ...
    [SerializeField] float spinDirection;
    ...
    void Update()
    {
        spinDirection = Random.Range(-1f, 1f);// gives the circle enemy a random spin direction
        if (spinDirection < 0)
        {
            plusInc = plusInc * -1; //rotates it anti-clockwise
        }
        else
        {
            plusInc = plusInc * 1; //rotates it clockwise
        }
